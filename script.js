...
const modalClose = codeModal ? codeModal.querySelector('.modal-close') : null;
const modalDownload = codeModal ? codeModal.querySelector('.btn-download-code') : null;
...
const pythonCodeScripts = {
    ...
    "Python Code/SolarSystem.py": `﻿import bpy
import math
import os
import random
...
def _resolve_project_dir():
    ...
    try:
        ...
        if os.path.isfile(script_path):
            candidates.append(os.path.dirname(script_path))
    except NameError:
        pass
    ...
    if bpy.data.filepath:
        candidates.append(os.path.dirname(bpy.path.abspath(bpy.data.filepath)))
        blend_models = bpy.path.abspath("//Models")
        if os.path.isdir(blend_models):
            return os.path.dirname(blend_models)

    candidates.append(r"C:\\Users\\Jonel Caisip\\Documents\\MMA BLENDER")
...
SCRIPT_DIR = _resolve_project_dir()
...
MODEL_FILES = {
    "Sun": ("SunModel.blend", "The Sun"),
    "Mercury": ("MercuryModel.blend", "Mercury"),
    "Venus": ("VenusModel.blend", "Venus"),
    "Earth": ("EarthModel.blend", "Earth"),
    "Mars": ("MarsModel.blend", "Mars"),
    "Jupiter": ("JupiterModel.blend", "Jupiter"),
    "Saturn": ("SaturnModel.blend", "Saturn"),
    "Uranus": ("UranusModel.blend", "Uranus"),
    "Neptune": ("NeptuneModel.blend", "Neptune"),
    "Pluto": ("PlutoModel.blend", "Pluto"),
}

def _hierarchy_bounds(root):
    ...
    if not found:
        ...
        size = Vector((dims.x, dims.y, dims.z))
...
def load_celestial_model(name, target_radius, location=(0, 0, 0)):
    blend_file, root_name = MODEL_FILES[name]
    blend_path = os.path.join(MODELS_DIR, blend_file)
    if not os.path.exists(blend_path):
        raise FileNotFoundError(f"Model not found: {blend_path}")
    ...
    if root is None:
        raise RuntimeError(f"Root object '{root_name}' not found in {blend_file}")
    ...
    _, size = _hierarchy_bounds(root)
    max_dim = max(size.x, size.y, size.z)
    if max_dim > 0:
        root.scale = (target_radius * 2.0 / max_dim,) * 3

    bpy.context.view_layer.update()
    center, _ = _hierarchy_bounds(root)
    root.location = Vector(location) - center
    scene_name = "Sun" if name == "Sun" else name
...
def build_system():
    ...
    sun_obj = load_celestial_model("Sun", SUN_RADIUS)
    ...
    for (pname, prad, orbit_r, orb_period, rot_period, axial_tilt, base_color) in PLANET_DATA:
        ...
        planet = load_celestial_model(pname, prad, location=(orbit_r, 0, 0))
        planet.parent = pivot
...
const getPythonCode = (codeUrl) => pythonCodeScripts[codeUrl] || null;
    ...
    document.querySelectorAll('.btn-view-code').forEach(button => {
        button.addEventListener('click', () => {
            ...
            if (codeUrl && title) {
                ...
                const hardcodedCode = getPythonCode(codeUrl);
                if (hardcodedCode) {
                    activeCodeContent = hardcodedCode;
                    modalCodeBlock.textContent = activeCodeContent;
                } else {
                    modalCodeBlock.textContent = '# Loading master code...';
                    fetch(codeUrl)
                        .then(res => {
                        ...
                        .then(text => {
                            activeCodeContent = text;
                            modalCodeBlock.textContent = text;
                        })
                        .catch(err => {
                            modalCodeBlock.textContent = '# Failed to fetch code: ' + err.message;
                        });
                }
